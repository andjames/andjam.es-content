...
For a republished Substack post, decide the canonical before publishing: omit `canonical` when andjam.es is the primary copy; set `canonical` to the Substack URL only while Substack remains primary. In the latter case, exclude that URL from the eventual site-wide sitemap rather than advertising two competing canonical locations.
